...
interface ArchiveState {
  ...
  results:Partial<Record<TestId,TestResult>>;
  history:HistoryItem[];
  pending:TestResult|null;
  setName:(name:string)=>void;
  setActiveCharacterId:(id:string|null)=>void;
  loadCharacter:(id:string,name:string,results:Partial<Record<TestId,TestResult>>)=>void;
  setPending:(r:TestResult|null)=>void;
  savePending:()=>void;
...
export const useArchive=create<ArchiveState>()(persist((set,get)=>({
  ...
  savePending:()=>{
    ...
    if(!r)return;
    set(s=>({
      results:{...s.results,[r.testId]:r},
      history:[{testId:r.testId,winner:r.winner,date:new Date().toISOString()},...s.history].slice(0,20),
      pending:null
    }));
  },
  saveResult:r=>set(s=>({
  ...
  reset:()=>set({name:'',activeCharacterId:null,results:{},history:[],pending:null})
}),{name:V9_KEY}));
