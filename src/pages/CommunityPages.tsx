...
import {getPublicCareerRecord,VILLAGES} from '../features/world';
import type {CareerRecord,CharacterLore,ShinobiCharacter,TimelineEvent} from '../types';
...
export function PublicProfilePage() {
  ...
  useEffect(() => {
    let live = true;
    setLoading(true);
    Promise.all([getPublicProfileBundle(slug),getPublicCareerRecord(slug)])
      .then(([bundle,careerRecord]) => {
        if(!live)return;
  ...
  return <div className={`screen public-profile theme-${theme} page-enter`}>
    ...
    <div className="public-hero">
      ...
      <div className="public-identity">
        <span className="eyebrow">PUBLIC SHINOBI ARCHIVE · V11</span>
        <h2>{displayName}</h2>
    ...
    {career&&<section className="public-lore"><span className="eyebrow">SHINOBI CAREER</span><h3>{career.operational_rank} · Level {career.level}</h3><div className="public-lore-grid"><div><span>Village Service</span><p>{career.village_id?`${VILLAGES[career.village_id].label} · ${career.village_reputation} reputation`:'Independent shinobi'}</p></div><div><span>Mission Record</span><p>{career.completed_missions} completed · {career.success_rate}% success rate</p></div></div>{career.village_id&&<Link className="mini-link" to={`/villages/${career.village_id}`}>View Village →</Link>}</section>}
