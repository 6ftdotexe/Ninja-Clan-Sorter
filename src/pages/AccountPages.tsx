...
export function HomePage(){const navigate=useNavigate();const {name,setName}=useArchive();const [draft,setDraft]=useState(name);return <div className="hero page-enter"><span className="eyebrow">SHINOBI IDENTITY ARCHIVE · V11</span><h1>Discover, build, and share your shinobi identity.</h1><p className="lede">Build your identity, join a living village, build a field career, forge techniques, deploy on missions, gain XP, form teams, and grow your shinobi inside a persistent world.</p><div className="namebox"><input value={draft} onChange={e=>setDraft(e.target.value)} placeholder="Shinobi name (optional)"/><button className="btn primary" onClick={()=>{setName(draft.trim());navigate('/archive')}}>Enter archive</button><button className="btn secondary" onClick={()=>navigate('/discover')}>Explore World</button></div><div className="meta"><div><strong>13</strong><span>IDENTITY TRIALS</span></div><div><strong>16</strong><span>CLANS</span></div><div><strong>∞</strong><span>PROFILE COMBOS</span></div><div><strong>AI</strong><span>CHARACTER ART</span></div><div><strong>V11</strong><span>LIVING WORLD</span></div></div></div>}
...
export function AccountPage(){
  ...
  return <div className="screen account-page page-enter">
    <PageHeader className="account-head" eyebrow="V11 · CLOUD ARCHIVE" title="Your Shinobi" description={user.email||''} actions={<div className="account-head-actions"><button className="btn secondary" onClick={()=>nav('/discover')}>Discover</button><button className="btn ghost" onClick={async()=>{await signOut();nav('/')}}>Sign out</button></div>}/>
