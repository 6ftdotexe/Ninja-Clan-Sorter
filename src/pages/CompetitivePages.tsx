...
import {equipmentBonuses,getTrainingProfile,listEquipment} from '../features/training';
...
import {ExamAdventure,type ExamAdventureResult} from '../lib/minigames';
import type {ChuninExamEntry,CombatStats,CompetitiveLeaderboardEntry,CompetitiveRecord,CompetitiveSeason,EquipmentInventoryItem,JutsuTechnique,StatKey} from '../types';
...
export function ExamsPage(){
  ...
  const [effectiveStats,setEffectiveStats]=useState<CombatStats>(stats);
  const [loading,setLoading]=useState(true);
  const [examPlay,setExamPlay]=useState(false);
  const action=useAsyncAction('Exam action failed.');
  const load=async()=>{
    setLoading(true);
    try{
      const active=await getActiveSeason();setSeason(active);
      if(activeCharacterId){const [current,competitive,past,techniques,training,equipment]=await Promise.all([getExamEntry(activeCharacterId),getCompetitiveRecord(activeCharacterId),listMyExamHistory(activeCharacterId),user?listJutsu(user.id,activeCharacterId):Promise.resolve([]),getTrainingProfile(activeCharacterId),listEquipment(activeCharacterId)]);setEntry(current);setRecord(competitive);setHistory(past);setJutsu(techniques);setEquipment(equipment);const gear=equipmentBonuses(equipment);const merged={...stats} as CombatStats;for(const key of Object.keys(merged) as StatKey[])merged[key]=Math.min(120,merged[key]+Number(training.bonuses[key]||0)+Number(gear[key]||0));setEffectiveStats(merged)}
  ...
  const advance=()=>setExamPlay(true);
  ...
  return <div className="screen exam-page page-enter">{examPlay&&entry&&entry.stage!=='complete'&&<ExamAdventure stage={entry.stage} title={EXAM_STAGES.find(x=>x.id===entry.stage)?.label||'Chūnin Exam Stage'} difficulty={entry.stage==='finals'?88:entry.stage==='preliminaries'?76:entry.stage==='survival'?64:55} combatStats={effectiveStats} jutsu={jutsu} equipment={equipment} summon={profile.summon} playerElement={profile.primaryChakra} onCancel={()=>setExamPlay(false)} onComplete={async(result:ExamAdventureResult)=>{setExamPlay(false);if(!result.passed){action.setError(`Exam event failed with ${result.score}. Train and attempt the stage again.`);return;}await action.run(async()=>{if(!entry)throw new Error('Register for the exam first.');return advanceChuninExam(entry.id)},{after:load,success:`Exam event cleared at ${result.score}; official evaluation resolved.`})}}/>}<PageHeader eyebrow="V11 · PHASE 3 · CHŪNIN EXAMS" title={`${profile.name}'s Chūnin Exam`} description="A playable exam event with written judgment, survival decisions, opponent encounters, and tournament battles layered over career readiness and technique mastery." actions={<Link className="btn ghost" to="/seasons">Season Standings</Link>}/><Feedback error={action.error} notice={action.notice}/>{loading?<p className="muted">Loading exam record…</p>:<>
