...
export function ExamsPage(){
  ...
  return <div className="screen exam-page page-enter">{examPlay&&entry&&<ActivityChallenge kind="exam" title={EXAM_STAGES.find(x=>x.id===entry.stage)?.label||'Chūnin Exam Stage'} difficulty={entry.stage==='finals'?88:entry.stage==='preliminaries'?76:entry.stage==='survival'?64:55} focus="Your execution must clear the stage gate before the official exam evaluation is resolved." onCancel={()=>setExamPlay(false)} onComplete={async(result:ActivityResult)=>{setExamPlay(false);if(!result.passed){action.setError(`Exam drill failed with ${result.score}. Train and attempt the stage again.`);return;}await action.run(async()=>{if(!entry)throw new Error('Register for the exam first.');return advanceChuninExam(entry.id)},{after:load,success:`Interactive stage cleared at ${result.score}; official evaluation resolved.`})}}/>}<PageHeader eyebrow="V11 · PHASE 3 · CHŪNIN EXAMS" title={`${profile.name}'s Chūnin Exam`} description="A multi-stage competitive evaluation of career readiness, training, mission experience, and technique mastery." actions={<Link className="btn ghost" to="/seasons">Season Standings</Link>}/><Feedback error={action.error} notice={action.notice}/>{loading?<p className="muted">Loading exam record…</p>:<>
