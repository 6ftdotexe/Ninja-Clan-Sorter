...
import type {ChuninExamEntry,CombatStats,CompetitiveLeaderboardEntry,CompetitiveRecord,CompetitiveSeason,JutsuTechnique,StatKey} from '../types';
...
export function ExamsPage(){
  ...
  return <div className="screen exam-page page-enter">{examPlay&&entry&&entry.stage!=='complete'&&<ExamAdventure stage={entry.stage} title={EXAM_STAGES.find(x=>x.id===entry.stage)?.label||'Chūnin Exam Stage'} difficulty={entry.stage==='finals'?88:entry.stage==='preliminaries'?76:entry.stage==='survival'?64:55} combatStats={effectiveStats} jutsu={jutsu} onCancel={()=>setExamPlay(false)} onComplete={async(result:ExamAdventureResult)=>{setExamPlay(false);if(!result.passed){action.setError(`Exam event failed with ${result.score}. Train and attempt the stage again.`);return;}await action.run(async()=>{if(!entry)throw new Error('Register for the exam first.');return advanceChuninExam(entry.id)},{after:load,success:`Exam event cleared at ${result.score}; official evaluation resolved.`})}}/>}<PageHeader eyebrow="V11 · PHASE 3 · CHŪNIN EXAMS" title={`${profile.name}'s Chūnin Exam`} description="A playable exam event with written judgment, survival decisions, opponent encounters, and tournament battles layered over career readiness and technique mastery." actions={<Link className="btn ghost" to="/seasons">Season Standings</Link>}/><Feedback error={action.error} notice={action.notice}/>{loading?<p className="muted">Loading exam record…</p>:<>
