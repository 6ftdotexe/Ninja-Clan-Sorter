...
import {EQUIPMENT_CATALOG,effectiveCombatStats,equipEquipment,equipmentSlotLabel,getTrainingProfile,listEquipment,purchaseEquipment,trainJutsuMastery} from '../features/training';
...
import {ActivityChallenge,MissionAdventure,type ActivityResult,type MissionAdventureResult} from '../lib/minigames';
...
export function ArsenalPage(){
  ...
  const [masteryPlay,setMasteryPlay]=useState<JutsuTechnique|null>(null);
  ...
  const train=(jutsuId:string)=>setMasteryPlay(jutsu.find(item=>item.id===jutsuId)||null);
  ...
  return <div className="screen arsenal-page page-enter">{masteryPlay&&<ActivityChallenge kind="mastery" title={`${masteryPlay.name} Mastery Drill`} difficulty={52+masteryPlay.masteryLevel*7} focus="Control, memory, and tactical execution determine whether this mastery session is effective." onCancel={()=>setMasteryPlay(null)} onComplete={async(result:ActivityResult)=>{const technique=masteryPlay;setMasteryPlay(null);if(!result.passed){action.setError(`Mastery drill failed at ${result.score}. No training points were spent.`);return;}await action.run(()=>trainJutsuMastery(technique.id,1),{after:refresh,success:`${technique.name} mastery drill scored ${result.score}.`})}}/>}
...
export function MissionsPage(){const nav=useNavigate();const {user}=useAuth();const {name,activeCharacterId,profile,stats}=useActiveShinobi();const [prog,setProg]=useState<ShinobiProgression|null>(null);const [missions,setMissions]=useState<ShinobiMission[]>([]);const [offer,setOffer]=useState<ShinobiMission|null>(null);const [missionPlay,setMissionPlay]=useState<ShinobiMission|null>(null);const [training,setTraining]=useState<TrainingProfile|null>(null);const [equipment,setEquipment]=useState<EquipmentInventoryItem[]>([]);const [missionJutsu,setMissionJutsu]=useState<JutsuTechnique[]>([]);const action=useAsyncAction('Mission action failed');
 const refresh=async()=>{if(!user||!activeCharacterId)return;const[p,m,t,e,j]=await Promise.all([getProgression(user.id,activeCharacterId),listMissions(user.id,activeCharacterId),getTrainingProfile(activeCharacterId),listEquipment(activeCharacterId),listJutsu(user.id,activeCharacterId)]);setProg({...p,level:levelFromXp(p.xp)});setMissions(m);setTraining(t);setEquipment(e);setMissionJutsu(j)};useEffect(()=>{void refresh().catch(e=>action.setError(errorMessage(e,'Could not load mission record')))},[user?.id,activeCharacterId]);
 ...
 const statKeys=Object.keys(stats) as StatKey[];const effectiveStats=effectiveCombatStats(stats,training,equipment);const buildBonus=statKeys.reduce((sum,key)=>sum+Math.max(0,effectiveStats[key]-stats[key]),0);
 ...
 const run=(fn:()=>Promise<unknown>)=>action.run(fn,{after:refresh});
 ...
 <section className="box"><div className="section-title"><div><span className="eyebrow">MISSION BOARD</span><h3>{active?'Active Deployment':'Request Assignment'}</h3></div>{!active&&<button className="btn primary" onClick={()=>setOffer(generateMission(profile,effectiveStats,level,Date.now()))}>Generate Mission</button>}</div>{active?<MissionCard mission={active} action={<><button className="btn primary" disabled={action.busy} onClick={()=>setMissionPlay(active)}>Enter Mission</button><button className="btn ghost" disabled={action.busy} onClick={()=>run(()=>abandonMission(user.id,active.id))}>Abandon</button></>}/>:offer?<MissionCard mission={offer} action={<><button className="btn primary" disabled={action.busy} onClick={()=>run(async()=>{await acceptMission(user.id,activeCharacterId,offer);setOffer(null)})}>Accept Mission</button><button className="btn ghost" onClick={()=>setOffer(generateMission(profile,effectiveStats,level,Date.now()+Math.random()*99999))}>Reroll</button></>}/>:<EmptyMessage>Generate an assignment matched to your current level and identity profile.</EmptyMessage>}</section>
 {missionPlay&&<MissionAdventure title={`${missionPlay.rank}-Rank · ${missionPlay.title}`} difficulty={{D:40,C:52,B:64,A:76,S:88}[missionPlay.rank]} objective={missionPlay.objective} location={missionPlay.location} rank={missionPlay.rank} category={missionPlay.category} combatStats={effectiveStats} jutsu={missionJutsu} equipment={equipment} summon={profile.summon} playerElement={profile.primaryChakra} onCancel={()=>setMissionPlay(null)} onComplete={async(result:MissionAdventureResult)=>{const mission=missionPlay;const r=resolveMission(mission,profile,effectiveStats,result.score);setMissionPlay(null);await run(()=>completeMission(user.id,mission,r.outcome,result.passed&&r.success))}}/>}
