...
import {ActivityChallenge,type ActivityResult} from '../lib/minigames';
...
export function MissionsPage(){const nav=useNavigate();const {user}=useAuth();const {name,activeCharacterId,profile,stats}=useActiveShinobi();const [prog,setProg]=useState<ShinobiProgression|null>(null);const [missions,setMissions]=useState<ShinobiMission[]>([]);const [offer,setOffer]=useState<ShinobiMission|null>(null);const [missionPlay,setMissionPlay]=useState<ShinobiMission|null>(null);const [training,setTraining]=useState<TrainingProfile|null>(null);const [equipment,setEquipment]=useState<EquipmentInventoryItem[]>([]);const action=useAsyncAction('Mission action failed');
 ...
 {missionPlay&&<ActivityChallenge kind="mission" title={`${missionPlay.rank}-Rank · ${missionPlay.title}`} difficulty={{D:40,C:52,B:64,A:76,S:88}[missionPlay.rank]} focus={`Complete the field drills to determine ${profile.name}'s mission execution.`} onCancel={()=>setMissionPlay(null)} onComplete={async(result:ActivityResult)=>{const r=resolveMission(missionPlay,profile,effectiveStats,result.score);setMissionPlay(null);await run(()=>completeMission(user.id,missionPlay,r.outcome,r.success))}}/>}
