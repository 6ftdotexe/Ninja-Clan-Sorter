...
export function BingoBookPage(){
  ...
  return <div className="screen bingo-book-page page-enter"><PageHeader eyebrow="V11 · PHASE 4 · BINGO BOOK" title="Known Missing-nin" description="Published rogue shinobi are ranked by bounty and notoriety. Threat classifications evolve as they participate in world events."/>{error&&<Feedback error={error}/>}<section className="world-section">{loading?<p className="muted">Loading classified records…</p>:entries.length?<div className="bingo-grid">{entries.map(entry=><Link className="bingo-card" key={entry.character_id} to={`/shinobi/${entry.public_slug}`}><div className={`bingo-threat threat-${entry.threat_class.toLowerCase()}`}>{entry.threat_class}</div><div className="mini-avatar">{entry.portrait_url?<img loading="lazy" decoding="async" src={entry.portrait_url} alt=""/>:'忍'}</div><div><small>{entry.rogue_title}</small><h3>{entry.name}</h3><p>{entry.clan||'Unknown clan'} · {entry.chakra_primary||'Unknown chakra'}</p><strong>{entry.bounty.toLocaleString()} Ryō bounty</strong><span>{entry.notoriety} notoriety{entry.last_known_village?` · formerly ${entry.last_known_village}`:''}</span></div></Link>)}</div>:<EmptyMessage>No published rogue shinobi are currently listed.</EmptyMessage>}</section></div>;
}
