...
## V11.5.6 Advanced Combat Engine

The shared `CombatEncounter` in `src/lib/minigames.tsx` now owns the cross-feature battle rules for elemental nature interaction, timed status effects, accuracy/evasion, rank-scaled jutsu cooldowns, equipment combat actions, summon support, charged ultimate techniques, and multi-phase boss behavior. Callers provide the already-derived effective combat stats plus equipped inventory, summoning contract, primary chakra, and mastered/loadout jutsu. This keeps the database authoritative for progression/rewards while allowing the application layer to provide a richer interactive resolution gate.

Elemental advantage follows the core five-nature cycle (Fire > Wind > Lightning > Earth > Water > Fire), with Wood/Ice receiving limited extension rules. Status effects are short-lived encounter state only; they are intentionally not persisted. Equipment actions are derived from existing equipped inventory and are usable once per battle, so no schema additions are required. The existing `ultimate` and `summoning` jutsu slots now have direct combat meaning.
